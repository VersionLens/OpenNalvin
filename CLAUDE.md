...
- Use `bun run dev` for the normal full-stack dev loop, or `bun run dev:api` / `bun run dev:web` when working on one side only. The API binds to `0.0.0.0:4210`; the Vite dev server binds to `0.0.0.0:4211` and proxies `/api` to Go.
- Use `bun run cli -- ...` for CLI/manual workflow checks and `go run . serve` only when you specifically want the raw server command.
- Common verification commands:
  ...
  - `bun run build:web`
- The Go binary expects the web assets to be built for embedded frontend serving, so if UI changes need to be reflected in the packaged app, run `bun run build:web` before rebuilding Go binaries.
- SQLite behavior matters in this repo. Prefer the repo's normal Go commands (`go test`, `go run`, `go build`) rather than custom SQLite setups; the project already ships a local `go-sqlite3` replacement with the expected features (FTS5 enabled).
...
The primary smoke-test loop is `bun run cli -- --workspace <ws> agent run -p "..."`. Key flags (see `cmd/agent_run.go`):
...
- `bun run cli -- --workspace <ws> agent tools list --json`
- `bun run cli -- --workspace <ws> agent tools run <tool> --input '{...}'`
- `bun run cli -- --workspace <ws> agent repl` for interactive sessions.
